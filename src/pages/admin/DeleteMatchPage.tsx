  ...
  const handleDeleteMatch = async () => {
    if (!slotId) {
      toast({ 
        ...
        description: 'Cannot delete match.' 
      });
      return;
    }
    try {
      ...
      if (response.ok) {
        ...
        throw new Error(errorData.msg || 'Failed to delete match');
      }
    } catch (error: any) {
      toast({ 
        variant: 'destructive',
        ...
        <div className="mb-6 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-2xl font-bold text-white">
              Delete Match {matchIndex ? `#${matchIndex}` : ''}
            </h1>
          </div>
        </div>
          ...
          <div className="bg-[#1A1A1A] border border-[#2A2A2A] rounded-lg p-8 space-y-6">
            ...
            <div className="flex justify-center gap-4 pt-4">
              ...
              <Button
                ...
                disabled={isDeleting}
                className="bg-[#FF4D4F] hover:bg-[#FF7875] text-white min-w-[120px]"
              >
...
export default DeleteMatchPage;
